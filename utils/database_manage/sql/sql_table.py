...
class Users(Base):
    ...
    personal_pass = Column(VARCHAR(32), index=True)
    birthday = Column(Date, nullable=True)
    email = Column(VARCHAR(128), nullable=True)
    full_username = Column(VARCHAR(128), nullable=True)
    selected_bot_skin = Column(Enum(BotSkins), nullable=True, index=True)
    selected_bot_lang = Column(VARCHAR(2), nullable=True, index=True)
