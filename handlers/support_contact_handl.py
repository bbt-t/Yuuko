...
async def contact_support_by_message(message: Message, state: FSMContext):
    text_msg: str = (
        ...
        'Do you want to write to the creator?'
    )
    kb = await sup_kb()
...
async def send_to_sup(call: CallbackQuery, state: FSMContext, callback_data: dict):
    ...
    await call.message.answer('пиши ...' if call.from_user.language_code == 'ru' else 'write ...')
...
async def get_message(message: Message, state: FSMContext):
    ...
    await message.bot.send_message(
        second_id,
        'Тебе пришло сообщение ->' if message.from_user.language_code == 'ru' else 'You received a message ->'
    )
    kb = await sup_kb(telegram_id=message.from_user.id)
    await message.copy_to(second_id, reply_markup=kb)
    await state.reset_state()

    await message.answer('Сообщение отправлено!' if message.from_user.language_code == 'ru' else 'Message sent!')
